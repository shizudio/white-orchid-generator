...
// (Text unification Phase A — ONE TEXT HOME, docs/text-unification-spec.md) The rail
// carries a single **Text** pill. The old post-type-specific label ("Caption" on a
// photo_logo) collided head-on with "+ Add text" — the client's ruling 1 — and the
// per-element pills ("Heading", "Body", …) split one system across N chips. Both are
// retired: every text on the design is a ROW inside the one Text panel.
const TEXT_PILL_LABEL = "Text";
...
export function useInspectorModel({
  ...
  // (Phase A) One home, one name. The pill and the panel header both read "Text",
  // whatever row is selected — the per-row class label carries the vocabulary now.
  const textInspectorTitle = TEXT_PILL_LABEL;
  ...
  // (Phase A) The added-element pill is gone, so `el:` keys never reach this rail —
  // an element is deleted from its ROW inside the Text panel (same reassurance line).
  ...
  const activeElements = useMemo(() => [
    ...
    // (Phase A · ONE TEXT HOME) The single Text pill. Its panel lists EVERY text on
    // the design as a row — filled legacy roles and added elements alike — with
    // "+ Add text" at the bottom of that same list.
    ...(postHasCopy ? [{ key: "text", label: TEXT_PILL_LABEL, icon: "T" }] : []),
    ...
    hasMedia, hasVideo, postHasCopy, hasRenderedLogo, archetypeId,
