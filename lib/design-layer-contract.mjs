...
// (Client ruling 2026-07-27: "once i add more than 1 shape, text cannot be shown,
// make sure text and logo are the top layers.") DECORATION moved from 70 (above
// content and marks) to 35 (below both): added shapes and decoration paint under
// the copy and the logo. Structural bands are untouched — an explicit brand
// profile may still move a structural overlay below content (doc §8).
...
export const DESIGN_RULES = Object.freeze([
  ...
  // (Client ruling 2026-07-26 — "text should always be visible, especially the heading
  // and according to info hierarchy") The TERMINAL rung of the §11 adaptation ladder for a
  // REQUIRED role. Floor size → extra lines → roomier variant → PAINT ANYWAY at the floor.
  // The role is on canvas (never dropped, never blank), but it exceeds the layout's honest
  // capacity — so the system says so, with executable remedies, instead of hiding the words.
  // WARNING, not blocking: painted-but-tight is an advisory state, and a blocking severity
  // here would re-create the dead end the ruling retired.
  rule("content.required-over-capacity", [DESIGN_LAYER_TYPES.CONTENT, DESIGN_LAYER_TYPES.LAYOUT, DESIGN_LAYER_TYPES.TYPOGRAPHY], "capacity", RULE_SEVERITIES.WARNING, RULE_ENFORCEMENTS.WARN, ["simplify-copy", "switch-layout", "edit-copy"]),
  ...
  // (Text unification Phase C — client ruling 5, DLC §12 "primary hierarchy remains
  // visually dominant") A lower-ranked text class paints visibly heavier than a higher
  // one — a body line out-shouting the heading. The system never assigns this: it can
  // only come from an explicit size/register PIN, which law 5 says is respected. So the
  // rule is an ADVISORY that NAMES the inversion and offers the two honest ways out —
  // release the pin that inverted it, or raise the voice that should lead.
  rule("typography.hierarchy-inverted", [DESIGN_LAYER_TYPES.TYPOGRAPHY, DESIGN_LAYER_TYPES.CONTENT], "hierarchy", RULE_SEVERITIES.ADVISORY, RULE_ENFORCEMENTS.WARN, ["release-size-pin", "raise-primary-role"]),
...
const LEGACY_FINDING_RULES = Object.freeze({
  ...
  "copy-over-capacity":"content.required-over-capacity",
