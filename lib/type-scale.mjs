 ...
 *
 * A single document-level control (typography.globalSizeStep: "S" | "M" | "L",
 * default "M") scales ALL text at once, but NOT uniformly — it changes the size
 * "based on information hierarchy". Verbatim intent: at L, "prioritize changing
 * title size … keep the rest in smaller size"; S "compresses inversely (less-
 * important roles compress first/harder)".
 *
 * Encoded as per-role STEP-RESPONSE WEIGHTS: what fraction of the family's full
 * S/M/L travel each role takes when the global control moves.
 *   grow   — fraction of (L_anchor − 1) a role takes at global L.
 *   shrink — fraction of (1 − S_anchor) a role gives up at global S.
 *
 * The two directions are DELIBERATELY INVERSE (the ruling's "inversely"):
 *   • at L the title travels the FULL step (grow 1.0) and lesser roles a
 *     restrained fraction — hierarchy contrast WIDENS as the design gets louder;
 *   • at S the title barely moves (shrink 0.3) and lesser roles compress hardest
 *     — hierarchy contrast still WIDENS as the design gets quieter.
 * So the signed response delta (mult − 1) obeys title ≥ subheading ≥ body at
 * EVERY global step (0 at M): title grows most / shrinks least; body grows least
 * / shrinks most. That is the property tested in type-scale.test.mjs.
 *
 * The weights are keyed by the role/class strings the codebase already paints:
 * legacy fm() roles (heading/subheading/content/highlight) and the added-element
 * classes (heading/subheading/body/caption/cta), plus edge furniture (eyebrow).
 * M is 1.0 for every role in every family → globalSizeStep "M" is pixel-identical
 * (fixture invariance, exactly like the S/M/L anchors).
 * ──────────────────────────────────────────────────────────────────────────── */

export const GLOBAL_SIZE_STEPS = Object.freeze(["S", "M", "L"]);

// Prominence tiers, most → least. Title-class roles travel the full step at L and
// hold their size at S; support roles a moderated ~60%; the small roles a
// restrained ~30% at L but the FULL compression at S. Numbers are the ruling's.
export const GLOBAL_STEP_RESPONSE = Object.freeze({
  // Title tier — the hero. Full grow at L, least shrink at S.
  heading:    Object.freeze({ grow: 1.0, shrink: 0.3 }),
  highlight:  Object.freeze({ grow: 1.0, shrink: 0.3 }), // overlay hero (texture_text/photo_logo)
  // Support tier — subheadings and body-support. ~60% of the travel, symmetric.
  subheading: Object.freeze({ grow: 0.6, shrink: 0.6 }),
  content:    Object.freeze({ grow: 0.6, shrink: 0.6 }), // legacy "support" role
  // Small tier — restrained growth, hardest compression (the "rest", by hierarchy).
  body:       Object.freeze({ grow: 0.3, shrink: 1.0 }),
  caption:    Object.freeze({ grow: 0.3, shrink: 1.0 }),
  cta:        Object.freeze({ grow: 0.3, shrink: 1.0 }),
  eyebrow:    Object.freeze({ grow: 0.3, shrink: 1.0 }),
});

// Unknown roles are treated as the smallest tier: a role we don't recognise must
// never out-grow the title, so it takes the most restrained grow / hardest shrink.
const DEFAULT_STEP_RESPONSE = Object.freeze({ grow: 0.3, shrink: 1.0 });

export function globalStepResponseFor(role) {
  return GLOBAL_STEP_RESPONSE[role] || DEFAULT_STEP_RESPONSE;
}

/**
 * The multiplier a role's size is scaled by for a given global step, in a family.
 *   globalStep "M" (or unknown)     → 1.0 for every role (pixel-invariant default)
 *   globalStep "L"  → 1 + grow(role)   × (L_anchor(family) − 1)   ≥ 1
 *   globalStep "S"  → 1 − shrink(role) × (1 − S_anchor(family))   ≤ 1
 * Monotonic per role (S < 1 < L) and hierarchy-preserving across roles (the
 * signed delta title ≥ subheading ≥ body at every step). This is what an UNPINNED
 * role/element follows; a pinned one ignores it (re-solve-around-pins, law 5).
 */
export function globalStepMult(role, globalStep = "M", family = "square") {
  if (globalStep !== "S" && globalStep !== "L") return 1.0;
  const el = TYPE_SCALE[clampFamily(family)].element;
  const w = globalStepResponseFor(role);
  return globalStep === "L"
    ? 1 + w.grow * (el.L - 1)
    : 1 - w.shrink * (1 - el.S);
}

/** Normalise any input to a valid global step; unknown/missing → "M" (the default). */
export function normalizeGlobalSizeStep(step) {
  return GLOBAL_SIZE_STEPS.includes(step) ? step : "M";
}
