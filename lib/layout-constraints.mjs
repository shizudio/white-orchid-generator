...
export function evaluateLayoutConstraints(capability, { zoneRects = {}, relationTests = {}, source = "preflight" } = {}) {
  ...
  const evaluated=[];
  const deferred=[];
  // Post-render, a CONTENT zone with no MEASURED box was dropped/undrawn (it never
  // reached the canvas). It must not be judged against its generous DECLARED allocation
  // rect — the ratified born-clean law evaluates only actually-drawn text roles (the
  // `textRoles` audit basis). Preflight keeps the declared rect (there is no render yet).
  const isPostRender = source !== "preflight";
  const rectFor = (zone) => {
    if (!zone) return null;
    const measured = zoneRects?.[zone.id];
    if (measured) return measured;
    if (isPostRender && zone.kind === LAYOUT_ZONE_KINDS.CONTENT) return null;
    return zone.geometry?.rect || null;
  };
  for (const relation of capability.relations) {
    const from=zones.get(relation.from),to=zones.get(relation.to);
    const fromRect=rectFor(from),toRect=rectFor(to);
