import { applyDesignCommand, DESIGN_COMMAND_TYPES, migrateDesignDocument } from "./design-document.mjs";
 ...
 * pins, motifs and furniture overrides). Layout-owned structural shapes follow
 * the replace rules: an untouched layout instance is replaced by the incoming
 * archetype frame, while a user-touched layout frame is a pin — it survives
 * and suppresses the incoming frame so two media hosts cannot compete.
 *
 * ── ADDED shapes (client ruling 2026-07-27, element-placement-spec §7c) ──────
 * "when there is a shape and user wants to switch layout, you should delete the
 * shape and change layout." With clearAddedShapes (every LIVE layout switch:
 * the Try-another-layout chip, the chat belt, a named layout ask — template
 * apply already re-skins via DOCUMENT_REPLACE), ALL added shape layers are
 * removed as part of the same atomic step. This deliberately SUPERSEDES the
 * former pins-survive-layout-swap behavior for added shapes. If a removed frame
 * hosted the photo, the SHAPES_REPLACE reducer re-resolves the media host so
 * the photo returns to the new layout's media zone (§3.4 machinery); ONE undo
 * restores shapes + prior layout together (the caller's patch snapshot
 * brackets the whole switch). Template/restore paths keep the default (false):
 * a stored document's own shapes are the design, not a live add.
 */
export function planArchetypeMaterializationWorkflow({
  ...
  currentShapes = [],
  typeLayoutDefault = {},
  clearAddedShapes = false,
}) {
  const shapesIn = Array.isArray(currentShapes) ? currentShapes : [];
  const isAddedShape = shape => shape && !shape.motif && shape.origin !== "layout";
  if (archetypeId === null || archetypeId === "none") {
    // Switching to the free layout is still a layout switch (§7c): added shapes
    // clear; the old layout's own shapes/motifs keep today's behavior.
    const commands = [
    ...
    const effects = [];
    if (clearAddedShapes && shapesIn.some(isAddedShape)) {
      commands.push({ type:DESIGN_COMMAND_TYPES.SHAPES_REPLACE, shapes:shapesIn.filter(shape => !isAddedShape(shape)) });
      effects.push({ type:EFFECT.CLEAR_SHAPE_SELECTION });
    }
    return [group(["archetypeId"], commands, effects)];
  }
  if (!materialized || typeof materialized !== "object" || !materialized.postType) return [];

  const retainedShapes = shapesIn.filter(shape => {
    if (shape?.motif) return false;
    if (shape?.origin === "layout") return shape.userTouched === true;
    return !clearAddedShapes;
  });
...
/**
 * Merge the CURRENT design's content into a template document — a template apply is a
 * RE-SKIN (client ruling 2026-07-25, upgrades 384b9bf).
 *
 * ALL current canvas content carries over — owner AND AI-authored copy (headline,
 * subtext, attribution, dateText, pill/eyebrow), plus every owner-added text element.
 * The template supplies only the DESIGN SYSTEM (layout geometry, archetype, palette
 * proposals, typography treatments, shapes/decor) — it contributes ZERO copy. Content
 * that no longer fits the new layout is handled downstream by the standard
 * complete-or-absent + readiness flow — it is never silently swapped for the template's
 * demo copy. Explicit owner pins (colour, logo) still ride the swap (pins law 5).
 */
export function mergeCurrentContent({ template, current } = {}) {
  ...
  // The template is a skin, not a script: the whole content block (all copy, its
  // authorship, and the owner's added elements) is the canvas's, never the template's.
  ...
  // (Item 5 — the contrast ladder must RE-RUN on a re-skin) An unpinned text colour and
  // backdrop are re-solved against the re-skinned design (reset to "auto") so a template's
  // stored ink can't render cream-on-cream over the merged content — the ghost text the
  // client reported. An owner PIN keeps the owner's explicit colour/backdrop (law 5).
  palette.text = curPins.textColorId ? cur.palette.text : "auto";
  palette.backdrop = curPins.backdropMode ? cur.palette.backdrop : "auto";
 ...
 * cannot drift between entry points. When a currentDocument is supplied, ALL of the
 * canvas's current content (owner and AI-authored) rides onto the template — the swap
 * re-skins the design instead of discarding the work in progress.
 *
 * ── INTENT (data-loss fix, 2026-07-27) ──────────────────────────────────────
 * This plan serves TWO callers with opposite content laws, and conflating them
 * destroyed client copy:
 *   · intent:"reskin"  — the template gallery. The canvas is the script, the
 *     template is the skin: current content wins (f5e1caa's ruling).
 *   · intent:"restore" — session bootstrap / opening a stored post. The STORED
 *     document IS the design; there is no "work in progress" to preserve. A
 *     merge here reads the freshly-mounted (empty) canvas as the owner's content
 *     and blanks every word of the restored post — which the debounced autosave
 *     then writes back over the good record.
 * Restore therefore never merges, whatever currentDocument the caller passes.
 */
export const TEMPLATE_APPLY_INTENTS = Object.freeze(["reskin", "restore"]);

/** The document whose content wins, or null when the stored design must open verbatim. */
export function resolveTemplateMergeSource({ intent = "reskin", currentDocument = null } = {}) {
  return intent === "restore" ? null : (currentDocument || null);
}
...
export function planTemplateApplicationWorkflow({
  ...
  // Current content survives the swap; run the merge AFTER materialization so it wins
  // over the archetype's palette/pin reset (a materialized layout clears pins first).
  // A RESTORE resolves to no merge source at all — the stored design opens verbatim.
  const mergeSource = resolveTemplateMergeSource({ intent, currentDocument });
  if (mergeSource) {
    resolvedDocument = mergeCurrentContent({ template:resolvedDocument, current:mergeSource });
  }
