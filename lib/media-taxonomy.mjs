...
// consent_status is DORMANT (client ruling 2026-08-03: "remove the consent
// category") — the column survives in the DB with its 'na' default, but no UI
// or validation reads it anymore.
//
...
// ── SESSION-LINEAGE GROUPING ─────────────────────────────────────────────────
// Groups images by the session they were created in (images.session_id).
// (2026-08-03) The Library's "By activity" view now groups by DETECTED activity
// instead — lib/activity-labels.mjs (the client's clarified meaning of
// "activity"). Session lineage stays recorded on rows and this grouping is
// retained for any lineage-based surface. Rows with no lineage land in one
// honest "Earlier / unlinked" group — we never fake groups for them.
