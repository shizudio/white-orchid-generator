...
import { DEFAULT_PHOTO_BRIEF } from './brand-defaults.js';
import { buildQcPrompt } from './style-dna.mjs';
...
// Single-sourced from lib/brand-defaults.js DEFAULT_PHOTO_BRIEF (P1 item e) — a
// brand_kit.photo_brief override can be passed in via the optional `photoBrief`
// param below; identical text when omitted (pixel/prompt-identity fallback).

function brandPhotoPrompt(scene, negativeSpace, photoBrief = DEFAULT_PHOTO_BRIEF) {
...
/* ── PHOTO QC (WP-U #4) ─────────────────────────────────────────────────────
   ...
   (Brand Style DNA — docs/brand-style-dna-spec.md) When the brand carries a
   style block, the SAME single call gains exactly ONE extra criterion: does
   the photo broadly match the block's lighting/palette/texture? The prompt
   comes from lib/style-dna.mjs buildQcPrompt — without a styleText it is
   byte-identical to the pre-style-DNA prompt, and a stray off_brand_style key
   in the verdict is IGNORED (absent = invisible). */
export async function qcPhoto(imageB64, { styleText = null } = {}) {
  const key = process.env.OPENAI_API_KEY;
  if (!key || !imageB64) return { pass: true, skipped: true };
  const model = process.env.OPENAI_AUDIT_MODEL || 'gpt-4o';
  const style = typeof styleText === 'string' && styleText.trim() ? styleText.trim() : null;
  try {
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: [
            { type: 'text', text: buildQcPrompt(style) },
    ...
    const offBrandStyle = !!(style && verdict.off_brand_style);
    const fail = !!(verdict.text_or_letters || verdict.poster_or_layout || offBrandStyle);
    return {
      pass: !fail,
      textOrLetters: !!verdict.text_or_letters,
      posterOrLayout: !!verdict.poster_or_layout,
      ...(style ? { offBrandStyle } : {}),
    };
  } catch (err) {
...
// (Style DNA spec §4 — verification) Preview the EXACT photographer-brief
// prompt a scene would submit, without submitting anything. Used only by the
// dev-gated dry-run in app/api/design-generate — the money-law-safe way to
// prove prompt assembly against a live server.
export function previewPhotoPrompt({ scene, negativeSpace } = {}) {
  return brandPhotoPrompt(String(scene ?? '').trim(), negativeSpace);
}
