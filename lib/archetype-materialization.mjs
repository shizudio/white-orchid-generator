import { PETAL_WINDOW_MASK_ASSET } from "./brand-defaults.js";
...
export function buildArchetypeMaterialization({
  ...
  if (photoFrame.type==="shapeMask"||photoFrame.type==="petalMask") {
    // (client ruling 2026-07-27) petalMask masks through the BRAND petal derived
    // from the ratified orchid mark (PETAL_WINDOW_MASK_ASSET) — the interim
    // shape-1 egg (the 2026-07-23 purge fallback) is superseded. shapeMask keeps
    // its per-variant shapeId (shape_cutout's rotating silhouettes).
    const assetId=photoFrame.shapeId||(photoFrame.type==="petalMask"?PETAL_WINDOW_MASK_ASSET:"shape-1");
