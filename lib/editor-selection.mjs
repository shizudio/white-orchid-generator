...
export function selectionInspectorKey(selection) {
  ...
  // (Slice 3) An ADDED text element rides a text selection whose role is the
  // synthetic `el:<uid>` key (docs/text-elements-spec.md §3; the canvas gesture +
  // rail chip both select it that way). It routes to its OWN element inspector,
  // not the shared "text" panel — so its inspector key IS the el:<uid> role. The
  // fixed archetype roles (hero/support/date/…) still share the "text" panel.
  if (selection.type === "text" && String(selection.role || "").startsWith("el:")) return selection.role;
