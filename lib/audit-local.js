...
// out of scope). Otherwise escalate along the spec §5 backdrop ladder based on
// severity AND what's already applied:
//   - a hard FAIL, or a zone already on a gradient that still clashes → propose
//     the SOLID band (0.92 opacity, uniform coverage — the strongest, spec §5),
//     since a high-variance photo region can defeat a feathered gradient in a
//     single cell even when the mean passes.
//   - a milder WARN with no scrim yet → propose the gradient (least intrusive
//     treatment that lifts contrast while keeping the photo visible).
//   - already on a band and STILL failing → no mechanical tweak left; return null.
function contrastFix(signal, severity) {
  ...
  if (severity === "fail" || mode === "gradient") return { backdropMode: "band" };
