...
export function mergeAuditIntoChecklist(check, auditFindings) {
  ...
  const mergedFormats = formats.map(fmt => {
    ...
    const elKeyOfLocal = (iss) => ledgerElementKey({ element: localIssueElement(iss), dimensionId: fmt.dimensionId });
    const indexByEl = new Map();
    issues.forEach((iss, i) => { const k = iss._elKey || elKeyOfLocal(iss); if (k) indexByEl.set(k, i); });
    for (const a of audits) {
      const elKey = ledgerElementKey(a.anchor);
