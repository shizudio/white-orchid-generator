...
export function computeReadyVerdict(signal, dimensionId) {
  ...
  if (zone && R) {
    ...
    if (logo && (inTop(logo) || inBottom(logo))) {
      // Story: the safe corner for a lockup is just below the top UI band, on the
      // left. logoPosition is a per-format pin (the pipeline writes logoByDim off the
      // master dim).
      push({
        id: "safe-area-logo",
        category: "safe-zone",
        message: `The logo is inside the ${zone.label} action zone and will be hidden by Instagram's controls. A clear corner keeps it on screen.`,
        fix: { dimensionId: dimId, logoPosition: inTop(logo) ? "mid-left" : "top-left" },
        severity: "fail",
      });
    }
  }
