...
import { HIERARCHY_RULE_ID, hierarchyInversions } from "./text-hierarchy.mjs";
...
export function runLocalAudit(signal) {
  ...
  if (floorLabels.length) {
    ...
    // (task #59) "use a roomier layout" becomes an EXECUTABLE remedy only when the
    // deferred pass verified a candidate that grows one of the floored roles.
    const _rv=signal.roomierLayout&&typeof signal.roomierLayout==="object"?signal.roomierLayout:null;
    const _grows=(_rv&&_rv.gains&&Array.isArray(_rv.gains.growsRoles))?_rv.gains.growsRoles:[];
    const _floorRoles=new Set([...minimumViolations.map(item=>item.role),...contractFloorRoles]);
    const _roomier=_rv&&_grows.some(role=>_floorRoles.size?_floorRoles.has(role):true)?{archetypeId:_rv.archetypeId}:null;
    findings.push({
      id: "type-size-floor",
      layer: "local",
      category: "type-size",
      ...
      fix: null, // the schema can't rewrite copy; the roomier remedy rides `roomier` when verified
      roomier: _roomier,
    });
  }
  ...
  if (overCapacityLosses.length) {
    ...
    // (task #59 — copy-fit Tier 0b action row) The roomier remedy is attached only when
    // the deferred pass VERIFIED a candidate whose gains grow one of the over-capacity
    // roles ("solver-verified candidate only — never offered blind", spec §4 remedy 3).
    const _rv=signal.roomierLayout&&typeof signal.roomierLayout==="object"?signal.roomierLayout:null;
    const _grows=(_rv&&_rv.gains&&Array.isArray(_rv.gains.growsRoles))?_rv.gains.growsRoles:[];
    const _overRoles=new Set(overCapacityLosses.map(item=>item.role));
    const _roomier=_rv&&_grows.some(role=>_overRoles.has(role))?{archetypeId:_rv.archetypeId}:null;
    findings.push({
  ...
  runHierarchyAdvisory(signal, findings);
...
// the placed dynamic-element list (each {uid,class,priority,text}); `roomier` the
// render's SOLVER-VERIFIED candidate ({archetypeId, gains} from the deferred
// layout-switch verification pass — task #59) or falsy. The switch-layout remedy
// carries the verified archetypeId so the action row can apply it as one undoable
// patch (never a dead label). Returns { lowest, simplifyUids, remedies }.
export function crowdingRemedies(placed, roomier) {
  ...
  const roomierArchetypeId = roomier && typeof roomier === "object" ? (roomier.archetypeId || null) : null;
  if (roomierArchetypeId) {
    remedies.push({ id: "switch-layout", label: "Try a roomier layout", kind: "switch-layout", archetypeId: roomierArchetypeId });
  }
...
function runCrowdingAdvisory(signal, findings) {
  ...
  // (task #59) roomierLayout is the deferred pass's SOLVER-VERIFIED verdict
  // ({archetypeId, gains}); the remedy is offered only when the candidate's gains
  // answer THIS need (a higher budget or a newly-placed element) — never blind.
  const verified = signal.roomierLayout && typeof signal.roomierLayout === "object" ? signal.roomierLayout : null;
  const gains = (verified && verified.gains) || {};
  const roomier = verified && (gains.budgetGrows || (Array.isArray(gains.placesElements) && gains.placesElements.length))
    ? verified : null;
  const { lowestLabel, remedies } = crowdingRemedies(placed, roomier);
  const tail = roomier
    ? `I can simplify it for you, drop the ${lowestLabel}, or try a roomier layout.`
    : (signal.roomierChecked
      ? `I can simplify it for you, or drop the ${lowestLabel} — I checked the other layouts and none is roomier for this content.`
  ...
  findings.push({
    ...
    fix: null,
    roomier: roomier ? { archetypeId: roomier.archetypeId } : null,
...
function runUnplacedElementFindings(signal, findings) {
  const elements = Array.isArray(signal && signal.contentElements) ? signal.contentElements : [];
  // (task #59) The roomier remedy attaches per element, only when the verified
  // candidate's gains actually PLACE that element — never a blind "try a roomier
  // layout" the switch can't back.
  const verified = signal && signal.roomierLayout && typeof signal.roomierLayout === "object" ? signal.roomierLayout : null;
  const placesElements = (verified && verified.gains && Array.isArray(verified.gains.placesElements)) ? verified.gains.placesElements : [];
  for (const el of elements) {
    if (!el || el.placed !== false || !el.uid) continue;
    const label = ELEMENT_CLASS_LABEL[el.class] || "text element";
    const roomier = verified && placesElements.includes(el.uid) ? { archetypeId: verified.archetypeId } : null;
    findings.push({
      ...
      message: roomier || !signal.roomierChecked
        ? `Your ${label} "${clipCopy(el.text || "", 40)}" didn't fit this format, so it's kept in storage and not shown here. Shorten it, remove it, or try a roomier layout.`
        : `Your ${label} "${clipCopy(el.text || "", 40)}" didn't fit this format, so it's kept in storage and not shown here. Shorten it or remove it — I checked the other layouts and none can place it without losing something.`,
      fix: null,
      roomier,
...
// (Text unification Phase C — client ruling 5 / DLC §12) The RELATIONAL HIERARCHY
// check, run on every solve from render truth: with more than one text voice on the
// canvas, the heading must read louder than the subheading, and both louder than the
// body/caption. The system never assigns an inverted ranking (its per-class sizes
// encode the order), so an inversion can only come from an explicit user PIN — which
// law 5 says is respected, never reverted. Hence ONE advisory that NAMES the inversion
// and offers the two honest ways out. Born-clean: a fresh generation ranks correctly,
// so nothing fires; a single text can never invert anything.
const HIERARCHY_VOICE = { heading: "heading", subheading: "subheading", body: "body text", caption: "caption" };
function runHierarchyAdvisory(signal, findings) {
  const entries = Array.isArray(signal && signal.textHierarchy) ? signal.textHierarchy : [];
  if (entries.length < 2) return;
  const inversions = hierarchyInversions(entries);
  if (!inversions.length) return;
  const worst = inversions[0];
  const lowerVoice = HIERARCHY_VOICE[worst.lower.class] || "text";
  const higherVoice = HIERARCHY_VOICE[worst.higher.class] || "text";
  const pinned = worst.lower.pinned ? `the ${lowerVoice}'s pinned size` : `the ${lowerVoice}`;
  findings.push({
    id: `hierarchy-inverted:${worst.lower.class}-over-${worst.higher.class}`,
    layer: "local",
    category: "composition",
    severity: "info",
    ruleId: HIERARCHY_RULE_ID,
    elementId: worst.lower.id || `text:${worst.lower.class}`,
    message: `Your ${lowerVoice} is reading louder than the ${higherVoice} — the eye lands on it first. I've kept ${pinned} as you set it. Set it back to Auto, or raise the ${higherVoice}, to put the ${higherVoice} back in the lead.`,
    fix: null,
    hierarchy: {
      lower: { id: worst.lower.id || null, class: worst.lower.class, px: worst.lower.px ?? null, pinned: !!worst.lower.pinned },
      higher: { id: worst.higher.id || null, class: worst.higher.class, px: worst.higher.px ?? null, pinned: !!worst.higher.pinned },
...
export function computeReadyVerdict(signal, dimensionId) {
  ...
  /* (TEXT UNIFICATION — ONE VOICE, law 1/2) The TEXT-SYSTEM advisories are not publish
     blockers, but they must have a SURFACE: a finding the engine computes and nobody can
     ever see is a dead rule (law 2 / M4). Only fail-severity findings gate readiness, so
     folding these in as non-blocking `info` rows reaches the advisor dot + the Export
     review list while `canPublish` is untouched. Scoped deliberately to the three
     element-system advisories — each can only exist AFTER the owner added text or pinned
     a size, so born-clean (a fresh design carries zero dots) is unaffected. Archetype
     drift advisories stay out: they are art-direction guidance on system-authored
     designs and would turn "Ready" into "to review" on a fresh generation. */
  const TEXT_SYSTEM_ADVISORY_RULES = new Set(["layout.whitespace-budget", "typography.hierarchy-inverted"]);
  for (const f of findings) {
    if (f.severity === "fail") continue;
    const isTextAdvisory = TEXT_SYSTEM_ADVISORY_RULES.has(f.ruleId) || String(f.id || "").startsWith("element-unplaced:");
    if (isTextAdvisory) push({ ...f, severity: "info" });
  }
