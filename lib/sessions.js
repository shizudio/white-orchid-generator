import { persistedCopyIsEmpty, persistedDesignToLegacyView } from './design-persistence.mjs';
...
// ── TOMBSTONE GUARD (data-loss defence, 2026-07-27) ──────────────────────────
// Defence in depth behind the restore fix (planTemplateApplicationWorkflow intent).
// A restore bug blanked the copy of a restored post and the debounced 2.5s autosave
// then wrote that emptied document over the client's good stored record — copy gone,
// locally AND in the shared cloud row. No save may turn a design that HAD words into
// one with none unless the owner emptied it deliberately ("Clear text", or clearing
// the fields one by one — both latch copyEmptiedByOwner in the editor).
//
// Fails OPEN by construction: an absent/unreadable prior, an absent next state, or an
// unparseable payload all resolve to "do not hold". The guard can only ever suppress
// the exact shape of write that destroys copy.
export function shouldHoldEmptyingSave(prior, next, { copyEmptiedByOwner = false } = {}) {
  if (copyEmptiedByOwner) return false;          // a deliberate clear must still save
  if (!prior || !next || !prior.state || !next.state) return false;
  if (persistedCopyIsEmpty(prior.state)) return false;   // nothing stored to lose
  return persistedCopyIsEmpty(next.state);
}
...
export async function saveSession(rec, { copyEmptiedByOwner = false } = {}) {
  if (HARNESS_MODE) return { configured: false, harnessSuppressed: true };   // (A1) suspend all persistence during a harness run
  if (!rec?.id) return { configured: false };
  const prior = localGetSession(rec.id);
  if (shouldHoldEmptyingSave(prior, rec, { copyEmptiedByOwner })) {
    // Loud, not silent: a held save means something upstream emptied the document
    // without the owner asking. The stored record is left exactly as it was.
    console.warn('[woSaveHold] refused to overwrite stored copy with an empty document', rec.id);
    return { configured: false, held: true, reason: 'empty-copy-overwrite' };
  }
...
// (Text Elements slice 4 — spec §5) The element-class-usage GENE. A stable, sorted,
// deduplicated token of the sanctioned classes present on this design's non-empty text
// elements (e.g. "body+cta+heading"), or null when the design carries none. This lets
// the like aggregate weight WHICH element classes the owner tends to keep — the tenth
// mirrored surface's payoff (buildGenes ↔ GENE_KEYS, guarded by mirror-check check 7).
export function elementClassesGene(state) {
  const elements = state && state.content && Array.isArray(state.content.elements) ? state.content.elements : null;
  if (!elements || !elements.length) return null;
  const classes = new Set();
  for (const el of elements) {
    if (el && typeof el.class === 'string' && typeof el.text === 'string' && el.text.trim()) classes.add(el.class);
  }
...
export function buildGenes(state, { sceneCategory } = {}) {
  ...
  return {
    ...
    elementClasses: elementClassesGene(state),
  };
}
