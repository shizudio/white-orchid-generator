...
// Strict OpenAI json_schema for { reply, patch }. Every patch field is optional
// (additionalProperties:false, empty required arrays) so the model can send a
// minimal delta — or, on the landing page, a fuller starting composition.
export const PATCH_JSON_SCHEMA = {
  ...
  properties: {
    ...
    patch: {
      type: 'object',
      additionalProperties: false,
      // No required keys: a patch may set any subset of these.
      required: [],
      properties: {
        postType: { type: 'string', enum: PATCH_OPTIONS.postType },
        dimensionId: { type: 'string', enum: PATCH_OPTIONS.dimensionId },
        headline: { type: 'string', maxLength: 100 },
        subtext: { type: 'string', maxLength: 140 },
        attribution: { type: 'string', maxLength: 100 },
        dateText: { type: 'string', maxLength: 60 },
        bgColor: { type: 'string', enum: PATCH_OPTIONS.bgColor },
        textColorId: { type: 'string', enum: PATCH_OPTIONS.textColorId },
        logoId: { type: 'string', enum: PATCH_OPTIONS.logoId },
        logoPosition: { type: 'string', enum: PATCH_OPTIONS.logoPosition },
        logoSize: { type: 'string', enum: PATCH_OPTIONS.logoSize },
        backdropMode: { type: 'string', enum: PATCH_OPTIONS.backdropMode },
        fontSizes: {
          type: 'object',
          additionalProperties: false,
          required: [],
          properties: {
            heading: { type: 'string', enum: PATCH_OPTIONS.fontStep },
            subheading: { type: 'string', enum: PATCH_OPTIONS.fontStep },
            content: { type: 'string', enum: PATCH_OPTIONS.fontStep },
            highlight: { type: 'string', enum: PATCH_OPTIONS.fontStep },
          },
        },
        addOverlay: {
          type: 'object',
          ...
          properties: {
            assetId: { type: 'string', enum: PATCH_OPTIONS.overlayAssetId },
            mode: { type: 'string', enum: PATCH_OPTIONS.overlayMode },
          },
        },
        removeOverlays: { type: 'boolean' },
      },
    },
  },
};
