...
export const PATCH_OPTIONS = {
  ...
  archetypeId: ['none', 'serif_word', 'editorial_split', 'big_number', 'full_bleed_duotone', 'floated_card', 'quote_margin', 'manifesto', 'documentary', 'label_headline', 'portrait_credential', 'motif_field', 'petal_window'],
  // DIMENSIONS[].id
  ...
  photoTreatment: ['none', 'duotone', 'duotoneLift', 'filmGrain', 'duotoneStrong', 'cleanGrain'],
  // photoFrame type (Commit 1 materialized visual) — the photo's shape/window.
  photoFrameType: ['none', 'card', 'petalMask'],
};
...
export const PATCH_FIELD_GUIDE = {
  ...
  archetypeId: 'Editorial composition archetype (starting point): serif_word (oversized serif hero), editorial_split (photo/text split), big_number (a date/number is the hero), full_bleed_duotone (photo full-bleed + whisper caption), floated_card (photo card floated on a solid field), quote_margin (quote with generous margin), manifesto (text-only), documentary (single clean photo), label_headline (eyebrow + headline), portrait_credential (portrait + credential), motif_field (solid field warmed by a few flat motifs), petal_window (photo revealed through an orchid mask). Use "none" to return to the free (non-archetype) layout. Setting an archetype also seeds its palette/treatment.',
  ...
  photoTreatment: 'How the photo is toned: none, duotone (deep-green luminance-preserving wash), duotoneLift (duotone + ivory paper glow), filmGrain (near-full-colour candid), duotoneStrong (heavy green, for masks/heroes), cleanGrain (near-full-colour, faint grain for cards). Usually set by choosing an archetype; tweak directly only on explicit request.',
  photoFrameType: 'Photo shape: none (fills its region), card (a floated rounded photo card on a solid field), petalMask (photo revealed through the orchid window). Usually set by the archetype.',
...
const AUDIT_COPY_FIELDS = ['headline', 'subtext', 'attribution', 'dateText', 'imagePrompt', 'scenePrompt'];
