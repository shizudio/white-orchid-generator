...
export const PATCH_OPTIONS = {
  ...
  // DEFAULT_OVERLAYS[].id (built-in shapes + accessories). 'orchid-petal' RETIRED
  // 2026-07-23 (off-brand, client ruling) — a legacy model output of it is silently
  // ignored (nullableEnum) and placed instances migrate to 'shape-1' on load.
  // 'petal-brand' ADDED 2026-07-27: the brand petal derived from the ratified
  // orchid mark (the Petal Window's mask, also placeable as a shape).
  overlayAssetId: ['petal-brand', 'shape-1', 'shape-2', 'shape-3', 'acc-arrow', 'acc-curve', 'acc-spark', 'acc-plus', 'acc-ring', 'acc-wave'],
  // overlay layer modes. (Line-art merge, client ruling 2026-08-18) 'outline'
  // and 'lineart' are ONE user-facing "Line art" mode: the editor re-points a
  // NEW add to the asset's correct internal renderer ('outline' = stroked
  // silhouette for filled shapes; 'lineart' = native-stroke keying for line
  // drawings). BOTH values stay in the enum — stored designs keep their exact
  // mode and render byte-identically (render-fingerprint law).
  ...
  // genuine shape layer now — "photo in the shape window" rides addOverlay
  // { assetId:'shape-1', mode:'frame' } like any shape (orchid-petal itself retired
  // 2026-07-23). A legacy model output of 'petalMask' is silently ignored
  // (nullableEnum), never a new special field.
  ...
  // (Font Ruling B) The register an element may PIN — the closed sanctioned voice set.
  // MIRRORS lib/typography-config.mjs SANCTIONED_REGISTERS, lib/text-elements.mjs
  // ELEMENT_REGISTERS, and the assistant route's ELEMENT_REGISTERS (the eleventh mirrored
  // surface — run mirror-check.sh after any edit). A pin only lands when it is sanctioned for
  // the element's class (heading: serif|heavySans; caption: serif|eyebrow; others single-voice).
  register: ['serif', 'heavySans', 'body', 'eyebrow', 'badge'],
};
...
export const PATCH_FIELD_GUIDE = {
  ...
  photoFrameType: 'Photo shape: none (fills its region) or card (a floated rounded photo card on a solid field). Usually set by the archetype. For a photo revealed through a brand shape (the organic shape window etc.) use addOverlay with mode "frame" — shape cutouts are ordinary shape layers.',
  ...
  addOverlay: 'Place a built-in brand shape. { assetId: one of shape-1..3/acc-*, mode: frame|outline|lineart|overlay }. shape-1 in "frame" mode makes the signature organic photo window. To SWAP a placed shape for a different one ("change the shape", "a different shape"), set removeOverlays true AND addOverlay with the new assetId in the same patch — removal happens before the add.',
  removeOverlays: 'Set true to clear all placed overlay shapes.',
  addTextElement: 'Add ONE new brand-governed text element (spec §5). { class: one of heading/subheading/body/caption/cta, text: the words to show }. Use this when the user asks to ADD a NEW piece of text that is NOT one of the existing roles — "add a caption saying X", "add a heading/subheading/body line saying X", "add a button/CTA saying X". Map the phrasing to the closest class: a headline-style line → heading; a supporting line → subheading; a paragraph/details → body; a small edge/eyebrow/date-like line → caption; a button/tag/CTA/pill → cta. An unrecognized ask ("add a quote") maps to the CLOSEST sanctioned class (body). CLASS-EXCLUSIVE (client ruling 2026-07-27): a design carries AT MOST ONE of each class — the reducer refuses an add whose class is already in use (see designState.elementClasses); do not set this field then, reply honestly instead (for body: encourage another paragraph inside the one Body). Space is never a reason to refuse — the layout solver places an allowed add; a class the solver cannot place cleanly in a format is kept in storage and named in readiness (never squeezed, never lost). Leave null when the user is editing existing copy (use headline/subtext/etc.) rather than adding a new element.',
  editElements: 'Edit EXISTING added text elements by their uid (spec §1). A list of { uid, class?, text?, sizeStep?, register? } — set only the fields to change per element. class must be a sanctioned transition; sizeStep is S/M/L only; register pins the typographic VOICE (Font Ruling B) and only lands when sanctioned for the element\'s class — a heading may pin serif or heavySans, a caption may pin serif or eyebrow (the tracked all-caps micro-label); other classes have a single voice. Set register to a value to switch the look ("make that heading a heavy sans"), or null to return to the class default. Leave the whole field null unless the user names an added element to change.',
  ...
  properties: {
    ...
    patch: {
      ...
      properties: {
        ...
        // Each entry compiles into content/set-element-{text,class,size,register}. class is a
        // sanctioned transition; sizeStep is S/M/L only; register is a sanctioned voice pin
        // (Font Ruling B). null = no element edits.
        editElements: {
          type: ['array', 'null'],
          items: {
            type: 'object',
            additionalProperties: false,
            required: ['uid', 'class', 'text', 'sizeStep', 'register'],
            properties: {
              uid: nullableString(64),
              class: nullableEnum(PATCH_OPTIONS.elementClass),
              text: nullableString(240),
              sizeStep: nullableEnum(PATCH_OPTIONS.elementSizeStep),
              register: nullableEnum(PATCH_OPTIONS.register),
            },
          },
        },
...
export function coerceFixToCategory(category, fix, state = {}) {
  ...
  for (const key of allowed) {
    ...
    // (M2 no-op guard, 2026-08-18 client report — "suggested edit doesn't change
    // anything") The vision model reasons from a rendered screenshot and can
    // misjudge what's already there, proposing a "fix" identical to the current
    // value. The prompt asks it not to; nothing enforced that server-side, so a
    // same-as-current fix sailed through to an executable "Fix" button that
    // silently did nothing when tapped. Drop any scalar field that already
    // matches — fontSizes (a per-role object) gets its own comparison below.
    if (key !== 'fontSizes' && fix[key] === state[key]) continue;
    out[key] = fix[key];
  }
  ...
  if (category === 'hierarchy' && out.fontSizes && typeof out.fontSizes === 'object') {
    ...
    // (M2 no-op guard) A per-role fontSizes fix that matches the current step for
    // every role is a no-op — strip it entirely rather than pass current values
    // straight through to applyPatch.
    if (!changed.length) {
...
export function editElementsHasChange(value) {
  return Array.isArray(value) && value.some(entry =>
    ...
    && (typeof entry.text === 'string' || PATCH_OPTIONS.elementClass.includes(entry.class)
        ...
        || PATCH_OPTIONS.register.includes(entry.register)));
}
