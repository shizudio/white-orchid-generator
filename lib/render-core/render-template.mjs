...
function logoRect(template, logoSlot, values, w, h) {
  ...
  /* (client ruling 2026-09-14 — WORDMARKS) THE MARK IS SIZED BY HEIGHT, not by
     width. Sizing by width gave every lockup the same footprint and therefore a
     wildly different optical weight: the brand's lockups run from 0.6:1
     (stacked) to 8.9:1 (the long wordmarks), so one width made the stacked mark
     216px tall and the wordmark 15px on the same 1080 frame. Height is what the
     eye actually reads, so height is what a template declares; the width then
     follows the asset's own proportions.

     `widthFrac` is still honoured for any slot that has not moved over, and the
     fallback below reproduces the old size exactly for a square lockup, so
     nothing shifts underneath a template that declares neither. Whether a
     variant's resulting WIDTH still fits its corner is not negotiated at paint
     time — it is a property of the sanctioned list, asserted by the guard in
     scripts/tests/logo-variants-fit.test.mjs. */
  const lh = (logoSlot.heightFrac ?? (logoSlot.widthFrac ?? 0.12) * 0.87) * w;
  const lw = lh / (ratio || 1);
