 ...
 */
export function autofit(ctx, { text, fontFor, box, maxLines, floorPx, lineRatio, ceilingPx = Infinity }) {
  ...
  const ceiling = Math.max(Math.floor(floorPx), Math.min(Math.floor(box.h / lineRatio), Math.floor(ceilingPx)));
  let size = ceiling;
  let lines = [];
 ...
 */
export function autofitTrackedCaps(ctx, { text, font, weight, tracking, box, maxLines, floorPx, lineRatio, ceilingPx = Infinity }) {
