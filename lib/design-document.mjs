...
const COMPOSITION_FIELD_SET = new Set(["postType", "archetypeId", "archetypeVariant"]);
...
export function normalizeShapeInstances(value) {
  ...
  return value.filter(isObject).map((shape, index) => ({
    ...clone(shape),
...
export function createDesignDocumentV1(source = {}) {
  ...
  return {
    ...
    content: {
      ...
      authorship: object(contentSource.authorship ?? source.copyAuthors),
    },
    composition: {
    ...
    shapes: normalizeShapeInstances(source.shapes ?? source.overlayLayers),
...
export function applyDesignCommand(document, command) {
  ...
  if (command?.type === DESIGN_COMMAND_TYPES.CONTENT_SET && CONTENT_FIELD_SET.has(command.field)) {
    ...
    const value = command.field === "archetypeVariant"
      ...
      : command.field === "archetypeId" ? (command.value ?? null) : text(command.value, "photo_logo");
    ...
    if (command.dimensionId && command.dimensionId !== command.masterDimensionId) {
      ...
      const masterLayouts = { ...current.typography.masterLayouts, [command.postType]:object(command.defaultLayout) };
      next = { ...current, typography:{ ...current.typography, masterLayouts } };
      changedPaths.push(`typography.masterLayouts.${command.postType}`);
    }
    ...
    changedPaths.push("media.source");
    if (reset) changedPaths.push("media.transforms");
    ...
    if (shape && !current.shapes.some(item => item.uid === shape.uid)) {
      next = { ...current, shapes:[...current.shapes, shape] };
    ...
    next = { ...current, shapes };
    changedPaths.push("shapes");
  } else if (command?.type === DESIGN_COMMAND_TYPES.SHAPE_UPDATE && command.uid && isObject(command.patch)) {
    const shapes = current.shapes.map(shape => shape.uid === command.uid ? { ...shape, ...object(command.patch), uid:shape.uid } : shape);
    if (shapes.some((shape, i) => shape !== current.shapes[i])) {
      next = { ...current, shapes };
    ...
    const shapes=current.shapes.map(shape=>{if(!shape.byDim?.[command.dimensionId])return shape;const byDim={...shape.byDim};delete byDim[command.dimensionId];return{...shape,byDim};});
    next={...current,
