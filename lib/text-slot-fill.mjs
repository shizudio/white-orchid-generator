...
/* ── class → the declared roles it may BECOME, in preference order ────────────
   Per the ratified mapping (Amendment 2026-07-27 ruling 1 — DEFAULT FILLS ARE
   HEADING + BODY): heading→hero; BODY→support — the secondary declared slot now
   presents and fills under the body class (its field, `subtext`, migrates as body
   in lib/text-elements.mjs, so the fill and the projection agree on one identity).
   caption→the caption/edge family (the eyebrow micro-label, then the date line);
   cta→pill. `subheading` now maps to nothing: filling support with a subheading
   would project back as a BODY row — a class mismatch wearing one uid — so a
   subheading add is always a genuine free element the solver seats. */
export const SLOT_FILL_ROLES_BY_CLASS = Object.freeze({
  heading:    Object.freeze(["hero"]),
  subheading: Object.freeze([]),
  body:       Object.freeze(["support"]),
...
export function slotFillTargetForClass(document, className) {
  ...
  for (const role of candidates) {
    ...
    // (ruling 1) A quote's support line IS its attribution — a small credit, class
    // caption, never a reading block. A body add on a quote must not become the
    // attribution, so it stays a genuine element there.
    if (className === "body" && role === "support" && postType === "quote") continue;
