...
export function resolveScenePointerTarget(sceneElements, x, y, {
  ...
  // Text/furniture, logo, and shape candidates are compared by PAINTED z so the
  // visually topmost element wins the pointer — not whichever type happens to be
  // checked first. Without this, a logo (z:50) sitting near/over a text role
  // (z:40, whose hit-box is also touch-inflated by minSize/padding) silently lost
  // every click to the text underneath it: a drag meant for the logo moved the
  // text instead, and the logo never budged (2026-08-18 client report — "sometimes
  // i can't drag it").
  ...
  const topmost = [copy, logo, shape].filter(Boolean).reduce(
    (best, candidate) => (!best || candidate.z > best.z ? candidate : best), null,
  );
  if (!topmost) { /* fall through to photo below */ }
  else if (topmost === logo) return { kind: "logo", element: logo, selected: false };
  else if (topmost === shape) return { kind: "shape", element: shape, selected: false };
  else if (topmost === copy) {
    return {
      kind: copy.type,
