...
/* ── OBSTACLE CLASSIFICATION (docs/design-layer-contract.md §7 collision matrix) ───
   The matrix — not whichever array an item happened to be collected in — decides
   whether a thing BLOCKS text or merely has to be RESOLVED under it:

     content × logo/legal mark        `B` blocking   → HARD
     content × content (another text) `R`, but no treatment makes text-on-text
                                       readable, so moving IS the remedy      → HARD
     content × structural shape/panel `R` resolve                             → SOFT
     content × media surface          `R` resolve                             → SOFT
     content × protected subject      `R`, carried separately as `focalBox`

   SOFT never means "ignored". placeTextElement runs a two-pass search that PREFERS
   candidates clear of every soft surface and lands on one only when nothing else
   clears — at which point the register-escalation ladder resolves legibility, per
   DLC §15 ("contrast is measured on the final surface beneath each role"; a
   structural panel is a thing content sits ON). "Resolve" means place-with-treatment,
   never refuse; refusal is reserved for GENUINELY exhausted candidates.

   Fail-closed: an unrecognised kind is HARD. A box that drew nothing (absent, or zero
   width/height) occupies nothing — an empty declared slot is opportunity, not
   occupancy (text-unification ruling 2). Pure. */
export const OBSTACLE_KINDS = Object.freeze({
  text: "hard",         // another drawn text role, or an already-placed element
  mark: "hard",         // logo / legal mark
  surface: "soft",      // the media (photo/video) window
  silhouette: "soft",   // structural shape, panel, colour field, decorative motif
});
export function classifyPlacementObstacles(items){
  const hard = [], soft = [];
  for (const item of (items || [])) {
    const box = item && item.box;
    if (!box || !(box.w > 0) || !(box.h > 0)) continue;
    (OBSTACLE_KINDS[item.kind] === "soft" ? soft : hard).push({ x:box.x, y:box.y, w:box.w, h:box.h });
  }
  return { hard, soft };
}
