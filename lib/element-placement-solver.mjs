...
// (orchestrator ruling) S/M/L step multipliers — the ONLY size freedom, applied to a
// class's per-render base px before the config is built. Kept small so a step stays
// inside the register's sanctioned range; the class floor still clamps the shrink.
export const ELEMENT_SIZE_STEPS = Object.freeze({ S: 0.82, M: 1, L: 1.25 });
export function elementStepPx(basePx, step){
  return basePx * (ELEMENT_SIZE_STEPS[step] || 1);
}
