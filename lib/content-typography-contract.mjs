...
export function evaluateContentTypographyContract(capability,{width,height,roleBounds={},textMetrics={},droppedRoles=[],overCapacityRoles=[]}={}){
  ...
  // (Client ruling 2026-07-26 — required content always paints) Render roles the painter
  // had to take past the layout's honest capacity to keep them VISIBLE. They are painted
  // (so they never appear in `dropped`), but the design owes the owner an honest advisory.
  const overCapacity=new Set((overCapacityRoles||[]).map(String));
  const violations=[];
  const atFloorRoles=[];
  const measured={};
  for(const role of capability.roles){
    ...
    // (Client ruling 2026-07-26) PAINTED, but past capacity — the honest advisory that
    // replaces the drop. Never emitted for a role that fits; never a `fail` (the words
    // ARE on canvas), and it carries the same remedies the drop finding used to offer.
    if(overCapacity.has(role.id)||overCapacity.has(role.renderRole)){
      violations.push({
        ruleId:"content.required-over-capacity",
        role:role.id,
        severity:"warn",
        evidence:{present:true,painted:true,overCapacity:true},
      });
    }
    if(fontHeight!=null){
