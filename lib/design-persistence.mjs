...
export function readPersistedDesignPayload(source) {
  if(source?.persistenceVersion===PERSISTED_DESIGN_VERSION&&source.document){
    const document=retireOffBrandShapes(migrateLegacyLayoutShape(repairStoredCopyStumps(migrateDesignDocument(source.document))));
  ...
  const document=retireOffBrandShapes(migrateLegacyLayoutShape(repairStoredCopyStumps(migrateDesignDocument(source||{}))));
  return {
    document,
