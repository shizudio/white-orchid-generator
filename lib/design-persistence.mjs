...
import { RETIRED_OVERLAY_ASSETS, RETIRED_OVERLAY_REPLACEMENT } from "./brand-defaults.js";

export const PERSISTED_DESIGN_VERSION=1;

// ── (client ruling 2026-07-23) Off-brand overlay art retirement ──────────────
// The 5-petal line "orchid-petal" artwork was retired from the paintable catalog
// (lib/brand-defaults.js RETIRED_OVERLAY_ASSETS). This LOAD-TIME adapter rewrites
// any placed instance in a stored design to the sanctioned organic silhouette,
// preserving its transform + mode + origin so the layout is byte-stable except
// for the silhouette itself — the off-brand art must not keep painting in old
// sessions either (client's explicit intent). Pure + idempotent: returns the
// SAME reference once no retired asset remains (so callers/tests can assert
// idempotence cheaply, matching migrateLegacyLayoutShape).
const RETIRED_OVERLAY_SET=new Set(RETIRED_OVERLAY_ASSETS);
export function retireOffBrandShapes(document){
  const shapes=document?.shapes;
  if(!Array.isArray(shapes)||!shapes.some(s=>s&&RETIRED_OVERLAY_SET.has(s.assetId)))return document;
  return {...document,shapes:shapes.map(s=>s&&RETIRED_OVERLAY_SET.has(s.assetId)?{...s,assetId:RETIRED_OVERLAY_REPLACEMENT}:s)};
}

// ── (Item 5 — the broken petal template) collapse redundant layout silhouettes ──
// Post-unification a design carries at most ONE layout-origin silhouette: the frame that
// HOSTS the photo (composition.mediaHostShapeId). A stored template that accreted a SECOND
// origin:"layout" shape — a fill/overlay silhouette left over from the old model, often
// tinted a field colour — paints it as a translucent coloured blob over the framed photo
// (the client's salmon blob). This load-time adapter keeps the single canonical silhouette
// — the media host, else the last frame-mode layout shape, else the last — and drops the
// redundant ones. Pure + idempotent: ≤1 layout shape returns the SAME reference (so
// callers/tests assert idempotence cheaply, and every clean design is untouched).
export function collapseRedundantLayoutShapes(document){
  const shapes=document?.shapes;
  if(!Array.isArray(shapes))return document;
  const layoutShapes=shapes.filter(s=>s&&s.origin==="layout");
  if(layoutShapes.length<=1)return document;
  const host=document.composition?.mediaHostShapeId;
  const keep=layoutShapes.find(s=>s.uid===host)
    ||[...layoutShapes].reverse().find(s=>(s.mode||s.renderMode)==="frame")
    ||layoutShapes[layoutShapes.length-1];
  const drop=new Set(layoutShapes.filter(s=>s!==keep).map(s=>s.uid));
  if(!drop.size)return document;
  return {...document,shapes:shapes.filter(s=>!(s&&drop.has(s.uid)))};
}
...
export function readPersistedDesignPayload(source) {
  if(source?.persistenceVersion===PERSISTED_DESIGN_VERSION&&source.document){
    const document=collapseRedundantLayoutShapes(retireOffBrandShapes(migrateLegacyLayoutShape(repairStoredCopyStumps(migrateDesignDocument(source.document)))));
  ...
  const document=collapseRedundantLayoutShapes(retireOffBrandShapes(migrateLegacyLayoutShape(repairStoredCopyStumps(migrateDesignDocument(source||{})))));
  return {
    document,
...
// ── (data-loss defence, 2026-07-27) Is there any copy left on this design? ────
// The tombstone guard in lib/sessions.js asks this of the STORED record and of the
// record about to overwrite it: a save that empties a design which had words is held
// unless the owner emptied it deliberately. "Copy" is every legacy role AND every
// text element (added elements included) — a design whose only words live in an
// added element is not empty. Pure; never throws (an unreadable payload reports
// "not empty" so the guard fails OPEN and can never block a legitimate save).
const COPY_TEXT_FIELDS = ["headline", "subtext", "attribution", "dateText", "microLabel", "pillText"];

export function documentCopyIsEmpty(document) {
  const content = document?.content;
  if (!content || typeof content !== "object") return true;
  for (const field of COPY_TEXT_FIELDS) {
    if (typeof content[field] === "string" && content[field].trim()) return false;
  }
  const elements = Array.isArray(content.elements) ? content.elements : [];
  for (const element of elements) {
    if (element && typeof element.text === "string" && element.text.trim()) return false;
  }
  return true;
}

export function persistedCopyIsEmpty(state) {
  try {
    return documentCopyIsEmpty(readPersistedDesignPayload(state).document);
  } catch {
    return false;   // unreadable → treat as "has copy" so the guard never blocks a save
  }
}
