/* ─────────────────────────────────────────────────────────────────────────
   ...
   1. THE PILL IS THE DOMINANT LINE, BELOW THE CAPTION. That inverts Classic's
      ...
      while the caption has a fixed two-line box at the floor and can never
      compete with its display scale.

      AND THE FIELD LABELS SAY SO. The small line is Caption and the dominant
      line is Heading — the same editing vocabulary used by every template.
   ...
   The caption's box is `2 x floorPx x lineRatio x 1.02`, allowing a practical
   40+ character message without lowering the legibility floor.
...
/* ── THE NUMBERS, DERIVED ONCE AT AUTHORING TIME AND RECORDED AS LITERALS ────
   CAPTION BOX — 2 lines at the floor, x lineRatio 1.28, x 1.02 slack:
     1080-short-side dims  73.44 x 1.28 x 1.02 x 2 = 191.76px
       → /1350 .14204   /1920 .09988   /1080 .17756
     landscape             61.20 x 1.28 x 1.02 x 2 = 159.81px → /900 .17756
...
export const TEMPLATE_CAPTION_BAND = assertValidTemplate({
  ...
  galleryPreview: {
    src: '/assets/post-template-samples/caption-band-portrait.png',
    alt: 'Forest Caption Band post featuring children playing on a rope course.',
  },
  ...
  /* THE MARK SITS ON THE PHOTO (client reference: top-right). Both TOP corners
     ...
     ONLY THE BOTTOM-RIGHT IS OFFERED, and that is geometry, not taste. Client
     ruling 2026-09-14 retired the mark's plate and gave the photograph the
     whole frame, which leaves nowhere on the image where an unknown photo can
     be trusted to carry a mark (measured: 1.04-1.1:1 against a floor of 3).
     The band's flat brand colour is the one field that can, so the mark lives
     there, in its own column beside the heading rather than over it.
  ...
  /* FIELD COPY — CONSISTENT SEMANTIC NAMES, NOT POSITIONAL DIRECTIONS.
     This template maps its small supporting line to Caption and its visually
     dominant line to Heading. The underlying closed slots stay stable for
     saved-content compatibility; the editing language stays stable for users. */
  slotLabels: {
    heading: { label: 'Caption', hint: 'Supporting detail in sentence case — up to two lines.' },
    pill: { label: 'Heading', hint: 'The main message. It is set in capitals.' },
  },
  ...
  registers: {
    heading: { face: 'body', weight: 300, caps: false, lineRatio: 1.28, align: 'left', ceilingScale: 1 },
    pill: { face: 'title', weight: 500, caps: true, tracking: 0.02, lineRatio: 1.02, align: 'left' },
  },

  slots: {
    // ── TEXT ────────────────────────────────────────────────────────────────
    /* THE CAPTION. Two lines in every dimension, providing a useful 40+
       character allowance without lowering the legibility floor. Optional —
       the heading can still stand alone. */
    heading: {
      present: true,
      charBudget: MEASURED_BUDGETS.heading.min,
      measured: MEASURED_BUDGETS.heading,
      dimensions: {
        portrait:  { present: true, required: false, maxLines: 2, charBudget: MEASURED_BUDGETS.heading.portrait,  box: { x: 0.06, y: 0.6813, w: 0.88, h: 0.14204 } },
        story:     { present: true, required: false, maxLines: 2, charBudget: MEASURED_BUDGETS.heading.story,     box: { x: 0.06, y: 0.7040, w: 0.88, h: 0.09988 } },
        square:    { present: true, required: false, maxLines: 2, charBudget: MEASURED_BUDGETS.heading.square,    box: { x: 0.06, y: 0.6214, w: 0.88, h: 0.17756 } },
        landscape: { present: true, required: false, maxLines: 2, charBudget: MEASURED_BUDGETS.heading.landscape, box: { x: 0.46, y: 0.3300, w: 0.52, h: 0.17756 } },
      },
    },
    ...
    /* ── THE PILL — THE BIGGEST TYPE ON THE DESIGN ────────────────────────────
       ...
       to a floor." The caption uses the light body register across two lines;
       the pill's box lets its short serif copy grow to ~137px, so "OPEN DAY"
       fills the band and "NOW ENROLLING" lands at ~110px. The visual hierarchy
       is verified by the renderer at every permitted copy length rather than
    ...
    pill: {
      ...
      dimensions: {
        portrait:  { present: true, required: false, maxLines: 1, charBudget: MEASURED_BUDGETS.pill.portrait,  box: { x: 0.06, y: 0.8363, w: 0.68, h: 0.10370 } },
        story:     { present: true, required: false, maxLines: 1, charBudget: MEASURED_BUDGETS.pill.story,     box: { x: 0.06, y: 0.8230, w: 0.68, h: 0.07292 } },
        square:    { present: true, required: false, maxLines: 1, charBudget: MEASURED_BUDGETS.pill.square,    box: { x: 0.06, y: 0.8104, w: 0.68, h: 0.12963 } },
        landscape: { present: true, required: false, maxLines: 1, charBudget: MEASURED_BUDGETS.pill.landscape, box: { x: 0.46, y: 0.5400, w: 0.40, h: 0.12963 } },
      },
    },
    ...
    /* ── THE PHOTOGRAPH ──────────────────────────────────────────────────────
       ...
       stated rather than left to be discovered:
         portrait  0.690 → 1080 x  932 (1.159:1) — the band gets 418px
         ...
         square    0.640 → 1080 x  691 (1.563:1) — the widest crop of the four,
    ...
    photo: {
      ...
      dimensions: {
        portrait:  { present: true, required: true, fit: 'cover', box: { x: 0, y: 0, w: 1.00, h: 0.656 } },
        story:     { present: true, required: true, fit: 'cover', box: { x: 0, y: 0, w: 1.00, h: 0.680 } },
        square:    { present: true, required: true, fit: 'cover', box: { x: 0, y: 0, w: 1.00, h: 0.600 } },
    ...
    logo: {
      ...
      /* Client ruling 2026-09-14: the plate is GONE. It existed only because the
         photo bled to every edge, leaving the mark nothing but photograph to sit
         on. The safe margin below gives the frame a field border, so the mark
         has brand colour underneath it and needs no panel of its own. */
      /* `pad` drops 0.07 -> 0.04. The 0.07 existed to fit the PLATE, which grew
         outward from the mark; with the plate retired the mark only needs its
         own air, and the wider inset was pushing it into the caption's column. */
      dimensions: {
        portrait:  { present: true, required: false, box: { x: 0.74, y: 0.8480, w: 0.22, h: 0.080 }, heightFrac: 0.090, pad: 0.04 },
        story:     { present: true, required: false, box: { x: 0.74, y: 0.8315, w: 0.22, h: 0.056 }, heightFrac: 0.090, pad: 0.04 },
        square:    { present: true, required: false, box: { x: 0.74, y: 0.8252, w: 0.22, h: 0.100 }, heightFrac: 0.090, pad: 0.04 },
        landscape: { present: true, required: false, box: { x: 0.46, y: 0.7500, w: 0.16, h: 0.119 }, heightFrac: 0.0625, pad: 0.04 },
      },
    },
  },
