  ...
  states: {
    ...
    photoOnly: {
      ...
      /* ── THE BLED, CENTRED PETAL (client rulings 2026-08-18) ─────────────
         ...
         VERTICALLY IT IS NOT CENTRED, AND CANNOT BE. A petal wide enough to
         bleed sideways is, at this near-square ratio, also taller than the
         canvas; centring it vertically would put its bottom edge under the
         mark. The HARD CONSTRAINT wins: the mark's corner stays flat field in
         every dimension, for BOTH sanctioned corners, so the whole overflow is
         taken at the TOP and the petal's bottom edge lands 12px above the
         mark's own top line. That is the size the bleed was reduced to.
         ...
         lands at:
           ...
           landscape  x  0.236 → 0.764   y -0.371 → 0.779
         Story bleeds sideways only — at 9:16 a petal that also cleared the top
         edge would have to be half again as wide as this one, and landscape is
         the mirror image: it bleeds off the top only, because at 16:9 a petal
         wide enough to reach the side edges would be over twice the canvas
         height. Each frame takes the overflow it can honestly carry.

         EVERY SANCTIONED SHAPE IS CHECKED HERE, not just the default: shape-2
         is WIDER than the window (1.1071:1), so it fills the box's full width
         and bleeds further sideways than shape-1 does. With the window centred
         that is symmetric too, which is what makes all three shippable — an
         earlier, right-offset version of this box pushed shape-2 off the left
         edge alone and lost the composition. */
      photo: {
        ...
        /* LANDSCAPE — 1.6× and vertically centred (client ruling 2026-08-18:
           "for lapscape, still bring the petal window frame down centralized
            vertically, make 1.6x bigger"). 0.6502 × 1.6 = 1.04032 and
           0.1150 × 16 = 1.84, centred on both axes: x = (1 - w)/2 = -0.02016,
           y = (1 - h)/2 = -0.42. It bleeds off the top AND the bottom, which is
           the point of this state. SEE THE REPORTED CONFLICT WITH THE MARK
           CORNER — the number is the client's and is honoured as given. */
        landscape: { box: { x: -0.02016, y: -0.42, w: 1.04032, h: 1.84 } },
      },
    },
  },
