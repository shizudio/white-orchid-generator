...
export const TEMPLATE_PETAL_WINDOW = assertValidTemplate({
  ...
  /* FIELD COPY (client ruling 2026-08-18: "also change this heading to just
     'Text'"). `heading` is the ONLY text field on this template, so the long
     descriptive label the surface uses by default is noise here. Classic keeps
     its own label — the two are independent now, which is what let this change
     land without touching the other template. */
  slotLabels: {
    heading: { label: 'Text' },
  },
