...
import { SANCTIONED_LOGO_ASSETS } from './logo-selection.mjs';
...
export const TEMPLATE_PETAL_WINDOW = assertValidTemplate({
  ...
  purpose: 'A required photograph seen through the brand petal on a quiet colour field, for when the picture is the post.',

  galleryPreview: {
    src: '/assets/post-template-samples/petal-window-portrait.png',
    alt: 'Sage Petal Window post featuring children playing on a rope course.',
  },
  ...
  allowedLogoAssets: SANCTIONED_LOGO_ASSETS,
  ...
  /* ── THE PANEL, IN THIS TEMPLATE'S ORDER (client ruling 2026-08-18) ───────
     "Petal window: Make window shape and photo selection the same section, it
      should be the first section."

     `window` is the merged section: the silhouette picker AND the photo
     control, one heading. And it is FIRST — above the heading field — because
     this template is PHOTO-FIRST. The petal IS the design (the photo is
     `required` in every dimension, and with no words at all the `photoOnly`
     state below is the whole post), so the window and its photograph are the
     primary decision, not an afterthought at the bottom of the panel.

     `colour` here is the colour-pair picker ALONE — unlike Classic, where
     colour and photo merge into one "Background" section. On this template the
     photo never sits behind the words; it sits inside the window. So the two
     genuinely are different decisions and are two sections, which is exactly
     why the order is declared per template rather than assumed by the panel. */
  panelSections: ['window', 'words', 'colour', 'mark', 'markPosition'],

  /* FIELD COPY. Editing vocabulary is deliberately shared across templates:
     the main message is always called Heading, never a layout-specific phrase. */
  slotLabels: {
  ...
  slots: {
    ...
    photo: {
      ...
      /* ── SHE MAY FRAME THE PICTURE (client ruling 2026-08-18) ────────────
         "i want to still shift around the image and resize the image. this
          only applies to petal window template."
         Here the photograph IS the design, so which part of it shows through
         the petal is a real decision. The WINDOW does not move and none of the
         geometry below changes — see the note on the contract's
         PHOTO_ZOOM_MIN/MAX for why that distinction is the whole line. */
      adjustable: true,
    ...
    logo: {
      present: true,
      dimensions: {
        portrait:  { present: true, required: false, box: { x: 0.05, y: 0.880, w: 0.22, h: 0.080 }, heightFrac: 0.090, pad: 0.05 },
        story:     { present: true, required: false, box: { x: 0.05, y: 0.915, w: 0.22, h: 0.056 }, heightFrac: 0.090, pad: 0.05 },
        square:    { present: true, required: false, box: { x: 0.05, y: 0.858, w: 0.22, h: 0.092 }, heightFrac: 0.090, pad: 0.05 },
        landscape: { present: true, required: false, box: { x: 0.05, y: 0.792, w: 0.10, h: 0.119 }, heightFrac: 0.0380, pad: 0.05 },
        /* Landscape's mark is smaller than the other three dimensions', and that
           is the photoOnly window's doing: at 1.38x it leaves only 0.092 of the
           width beside it, so a 2.40:1 wordmark has to come down to 0.038 height
           to stay off the petal. The alternative was a mark that overlaps the
           window in one state and not the other, which is worse. */
      },
    },
  },
  ...
  /* ── THE TWO AUTHORED STATES (client ruling 2026-08-18) ───────────────────
     ...
     The client's original reference was a near-full-bleed, optically centred
     petal with NO words. That is now a FIRST-CLASS LAYOUT rather than a
     side-effect of leaving the heading blank.

     TWO LAYOUTS A DESIGNER DREW — NOT A RULE. Read the note in
     lib/templates/template-contract.mjs before touching this. Nothing here is
     computed, scaled, interpolated or derived from the other state: every
     number below was drawn and then verified in the render core
     (scripts/tools/verify-template-two.mjs). A THIRD state, or any blend
     between these two, is where this stops being two drawings and becomes a
     solver — the validator refuses it, and it needs a client ruling.

     WHICH ONE PAINTS is HER CHOICE (client ruling, same day): a text on/off
     toggle beside the field, read in the render core as a slot value
     (`showText` → resolveTemplateState), generically, from `states` and never
     from a template id. It is deliberately NOT inferred from whether the copy
     is empty — a layout that flipped while she cleared a line to retype it is
     the surprise class this app exists to remove. Text ON with no words paints
     an empty band, honestly and stably.

     DECISION 2 ABOVE IS UNCHANGED, not quietly contradicted. "The petal DOES
     NOT MOVE OR SHRINK when a heading appears" was written against a REFLOW —
     geometry recomputed from CONTENT, which is the solver in data. That is
     still banned and still true: within `withHeading` the petal is identical
     for an empty heading and a full one, and
     scripts/tools/verify-template-two.mjs still proves it byte for byte. This
     is a second DRAWING she switches to, not this drawing responding to what
     she typed.

     WHAT MOVES, AND WHAT DELIBERATELY DOES NOT:
       · heading  — ABSENT in photoOnly. There are no words, so there is no band;
                    the petal takes the air the band was holding.
       · photo    — a bigger petal, horizontally centred, sitting optically
                    centred in the field ABOVE the mark, clearing the mark's
                    corner by 34–137px in every dimension. Portrait/story go to
                    0.88 of the width (from 0.80), square to 0.74 (from 0.578),
                    and landscape leaves the petal-left/field-right split
                    entirely — with no text there is no field to sit beside, so
                    it centres like the other three.
       · logo     — UNCHANGED, on purpose. The mark is bottom-left at the same
                    size in both states; it is the one fixed thing the eye can
                    hold across the switch. The contract refuses a state row
                    that changes nothing (M4), so an unchanged slot is declared
                    by ABSENCE here rather than by a duplicated copy that could
                    silently drift.
     Budgets, `required`, `fit`, `maxLines` and the scrim are untouched: the
     contract only lets a state override geometry, so §7.1 needs no second copy. */
  states: {
    // The baked geometry above IS this state. It overrides nothing — a second
    // copy of those numbers could only drift from them.
    withHeading: {},

    photoOnly: {
      // No words → no band. Not a shrunken band, not an empty one: absent.
      heading: {
      ...
      /* ── THE BLED, CENTRED PETAL (client rulings 2026-08-18) ─────────────
         "the text off petal can be even bigger, overflowing the frame like
          referenced" … then "no i need the petals to be centralized".

         So the window OVERFLOWS the canvas and the frame crops it — the box is
         deliberately bigger than the canvas — and it is CENTRED: x = (1 - w)/2
         in every dimension, so the bleed is symmetric left and right rather
         than pushed to one side. The contract allows an out-of-bounds box for a
         PHOTO slot only (see checkPerDimRow).

         THE ONE THING THE BLEED MAY NEVER DO IS REACH THE MARK. On the three
         tall frames the whole overflow is therefore taken at the TOP and the
         sides, and the petal's bottom edge lands 12px above the mark's own top
         line; landscape gets there a different way (see its own note below).
         The mark's corner stays flat field in every dimension, for BOTH
         sanctioned corners — bottom-left and bottom-right.

         The numbers below are the BOX. What paints is the silhouette CONTAINED
         in it at the asset's own proportions, so the DEFAULT petal (shape-1)
         lands at:
           portrait   x -0.030 → 1.030   y -0.168 → 0.871
           story      x -0.030 → 1.030   y  0.179 → 0.909
           square     x -0.030 → 1.030   y -0.451 → 0.847
           landscape  x  0.142 → 0.858   y -0.280 → 1.280
         Each frame takes the overflow it can honestly carry: story bleeds
         sideways only, because at 9:16 a petal that also cleared the top edge
         would have to be half again as wide as this one; landscape bleeds top
         and bottom only, because at 16:9 a petal wide enough to reach the side
         edges would run straight through both mark corners.

         EVERY SANCTIONED SHAPE IS CHECKED HERE, NOT JUST THE DEFAULT, and that
         is not a formality — it is what the geometry is authored around.
         shape-2 is WIDER than the tall frames' windows (1.1071:1), so it fills
         their full box width and bleeds further sideways than shape-1 does;
         with the window centred that is symmetric too, which is what makes all
         three shippable. An earlier, right-offset version of this box pushed
         shape-2 off the left edge alone and lost the composition, and a naive
         landscape box let shape-2 run under the bottom-right mark. */
      photo: {
        portrait:  { box: { x: -0.1525, y: -0.1676, w: 1.3050, h: 1.0387 } },
        story:     { box: { x: -0.1525, y: 0.1790, w: 1.3050, h: 0.7303 } },
        square:    { box: { x: -0.1525, y: -0.4512, w: 1.3050, h: 1.2984 } },
        /* ── LANDSCAPE — 1.38x, CENTRED, RE-PROPORTIONED ──────────────────
           WHY 1.38x. The client first ruled 1.6x ("make 1.6x bigger"), which
           put the petal under the mark's corner. Measured over 11 library
           photos x 5 sanctioned marks x 2 sanctioned corners: only 2 of 30
           combinations cleared all four dimensions, and sage (the default) and
           forest had NO clean mark at all. At 1.38x the corner keeps flat
           field and all 16 tone-appropriate combinations pass on BOTH corners
           — the verdict returns to the pair's own flat-field ratio, i.e. it
           stops depending on the photograph instead of "passing on most
           photos". A self-contained badge was refuted by the same sweep
           (p-circle is a green-on-light lockup: 7/11 failures on sage), as was
           per-dimension mark placement. Client ruled 1.38x on that evidence.

           WHAT IS SHIPPED IS NOT A NAIVE 1.38x SCALING, AND THE DIFFERENCE
           MATTERS. Scaling the 1.6x box by 0.8625 on both axes gives
           {x: 0.05136, y: -0.2935, w: 0.89728, h: 1.587}. That box is 0.897 of
           the canvas wide and CENTRED, so its left edge lands at 0.0514 —
           inside the bottom-left mark's own column (0.05 - 0.13), and its
           right edge at 0.9486, inside the bottom-right mark's (0.87 - 0.95).

           THE BOX IS WHAT MUST CLEAR THE MARK, NOT THE PAINTED SILHOUETTE.
           A shape is CONTAINED in the window at its own proportions, so how
           far it insets from the box depends on WHICH shape she picks. The
           default (shape-1, 0.8164:1) insets ~135px and would have cleared the
           corner by ~9px; shape-2 is 1.1071:1, WIDER than that box, so it
           fills the box's full width and insets by nothing. Measured at the
           naive geometry: shape-1 16/16 clean, shape-3 16/16, and shape-2
           8/16 — every bottom-RIGHT combination failed, field under the mark
           collapsing to 0.04. Gating the painted rect would have shipped a
           window that breaks the moment she picks a different sanctioned
           petal; gating the BOX is shape-agnostic and catches it.

           SO THE BOX IS RE-PROPORTIONED RATHER THAN RESIZED: narrow enough
           that EVERY sanctioned silhouette is width-limited inside it and
           therefore paints at exactly 1146px wide — clearing both mark columns
           by 0.012 (19px) — and correspondingly taller, so the AREA is the
           1.38x area unchanged (0.716 x 1.9888 = 1.424 = 0.89728 x 1.587).
           The painted petal is 1146 x 1403 against the naive 1166 x 1428:
           1.7% narrower, the same picture to the eye.

           WHAT SHE SEES, IN WORDS: a large petal centred on BOTH axes, cropped
           by the top and bottom edges of the frame, with quiet field down each
           side and the mark sitting on flat colour in its corner. That is the
           composition the client approved; only the box's proportions moved,
           and only so that every petal she can pick keeps that promise. */
        landscape: { box: { x: 0.1420, y: -0.4944, w: 0.7160, h: 1.9888 } },
      },
    },
  },
