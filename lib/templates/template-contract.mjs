...
/* Per-template FIELD COPY (client ruling 2026-08-18: "also change this heading
   to just 'Text'"). The same slot can honestly want a different name in two
   templates — on Petal Window `heading` is the only text field there is, so the
   long descriptive label is noise; on Classic it is the line that carries the
   whole post and the long label earns its place. A template may therefore
   override the surface's default label and hint for a slot it paints.
