...
// The two colour classes a pair can belong to. `logoAssets` is already keyed by
// this; the photo scrim is keyed the same way — a LOOKUP TABLE, not a rule: a
// dark ink needs a light scrim and a light ink a dark one, and which one applies
// is read off the pair, never computed by the template.
...
export function validateTemplate(template) {
  ...
  if (!slots || typeof slots !== 'object' || Array.isArray(slots)) {
    ...
    for (const name of SLOTS) {
      ...
      // The photo TREATMENT as data (§6.2 / client amendment 2026-08-18): one
      // fixed scrim per colour class, colour + opacity. The render core ALWAYS
      // paints it over a photo, so an absent or transparent scrim is rejected —
      // a scrim that does nothing would be a dead declaration (M4).
      if (name === 'photo') {
        const scrim = slot.scrim;
        if (!scrim || typeof scrim !== 'object' || Array.isArray(scrim)) {
          errors.push(`${at}.scrim: a present photo slot must declare the fixed scrim, keyed by colour class (${COLOUR_CLASSES.join(', ')})`);
        } else {
          for (const klass of COLOUR_CLASSES) {
            const row = scrim[klass];
            const sat = `${at}.scrim.${klass}`;
            if (!row || typeof row !== 'object') { errors.push(`${sat}: required { colour, opacity }`); continue; }
