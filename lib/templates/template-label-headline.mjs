...
export const TEMPLATE_LABEL_HEADLINE = assertValidTemplate({
  ...
  purpose: 'A quiet statement tile: a small tracked label, one serif sentence that carries the whole post, and a short line underneath. A photo behind the words is optional — leave it out and the tile is a plain colour field. Best when the words are the point.',
  ...
  slots: {
    ...
    // scrim per colour class. There is no ladder and no adaptive opacity — the
    // scrim is a declared number, and whether the result is readable is MEASURED
    // off the painted canvas afterwards (lib/render-core/backdrop-contrast.mjs).
    //   0.72 is MEASURED, not taste. Swept over the brand's whole live photo
    //   library — 131 images, every text box, both classes — by
    //   scripts/tools/scan-library-backdrop.mjs:
    //     · light scrim → 0 of 131 fall below the 4.5 floor (worst 4.75)
    //     · dark scrim  → 12 of 131 fall below it (the bright ones)
    //     · a near-black photo under the light scrim lands 4.43, and a
    //       near-white one under the dark scrim 4.02 — both still refused.
    //   So an ordinary photo just works, and an extreme one is honestly held.
    //   Below 0.70 ordinary photos start failing; above 0.75 the extremes start
    //   passing and the check stops being a check.
    photo: {
      present: true,
      scrim: {
        light: { colour: P.whiteSmoke, opacity: 0.72 },
        dark: { colour: P.burnham, opacity: 0.72 },
      },
      dimensions: {
