...
- **Owner rails for solver-placed elements — PARTIAL / P3-pending.** The date / eyebrow / badge are **already draggable** via the existing free-pin machinery (they report drawn boxes into `roleBounds`, and the soft-snap drag hit-tests + snaps them to the anchor set), and a solver-placed **badge** is not re-solved once a drag offset exists (law-5 pin). **Not yet landed:** (a) a *guaranteed* base-freeze so re-solve can **never** move a dragged date/eyebrow (today the drag is a centre-delta over the solver's chosen candidate — stable while that candidate stays chosen, not guaranteed under a re-solve that picks a different candidate); (b) the pinned-placement **ledger dot** — the rescued eyebrow and the badge are not yet in the audit's `textBoxes`, and there is no dedicated "you pinned this into a hard-restraint violation" finding (the 4f9afcd `logo-legibility` `fix:null` pattern is the template). Stopped short here deliberately: the finding lives in the ratified one-voice ledger (`lib/audit-local.js`) and its gate (constructed-violation → honest dot + no auto-fix; drag→pin→re-solve invariance; undo-reverts) could not be verified to the operating-manual's runtime bar this session without risking born-clean / one-voice.
