...
**Two things to know before you trust a line number.** (1) Anchors in this doc are **function and constant names**, not line numbers: the renderer and editor orchestration are split across `components/Generator.jsx`, focused components, hooks, and pure `lib/` contracts, so line numbers drift — search the name. (2) The decor / overlay-shape subsystem is now reducer-backed and uses stable scene identities, but its visual thresholds still belong to the renderer; verify compose-vs-avoid, early-vs-late drawing, and shape contrast gates against the named functions before changing them.
...
Every archetype declares its parts as **canvas-fraction boxes** (`{x, y, w, h}` in 0–1 space, so they scale to any format): a **hero** (the headline), **support** (the caption/body), a **micro-label** (the eyebrow), a **logo** (position + size), and optionally a **photo**, a **card** or **mask** (clip containers). There are **exactly three text roles** — hero, support, micro-label. The "pill" here is not a fourth text role; it's a *rendering treatment* (the message-pill archetype draws hero+support inside a rounded card). The accent **badge** (a small filled lozenge with a caps label) is a separate thing — a piece of furniture, below.

**The universal element contract & placement solver (P1 — `docs/element-placement-spec.md` §2).** An archetype's declared `elements{}` are no longer permissions — they are **priors** (the art-directed preferred spot). Three secondary elements have adopted a shared **placement solver** (`placeElement` → `placeTextElement`), generalized from the logo's own candidate/filter/score loop:
- **the date line** (P1 1b, `ae2a8c6`), **the eyebrow / micro-label** (P1 2b-i, `e2ed8b8`), and **the accent badge / pill** (P1 2b-ii, `599f62c`).
For each, the element's **current position is candidate #1** — the archetype prior (or, for a user-added pill, the spot it lands at) — kept **byte-identical when it lands**. The solver runs **only when that prior would drop**: it then walks relational + generic-grid anchors (inset by the platform-safe zones) through the ratified **register-escalation ladder** (ink-flip → heavier sanctioned weight → the robust brand face → size up → band) and pins the element to the first clean, in-bounds, collision-free spot — as a quiet corner/edge line rather than dropping it. The badge is opaque (self-legible), so its ladder is a single rung — the solver just finds clean placement. A genuine refusal (no candidate is clean) leaves the element honestly dead, exactly as before. **"Not shown in this layout" is retired** for the date, eyebrow, and badge — they now land on any design that has room, or surface the honest constraint, never silently drop because an archetype "didn't author" them. The solver fires **only on non-empty explicit content**, so autonomous generations never sprout one of these elements (born-clean holds). The hero and photo remain archetype-owned (they *are* the composition).

**Furniture** — the small graphic marks — is a **closed vocabulary of five**: rule, underline, index, counterweight, badge. One painter draws all of them; adding a sixth kind means a new branch in that painter. A URL is not its own element — it lives inside support or the counterweight. (The badge is the one furniture kind that also adopts the placement solver above; the other four stay auto-placed-or-skipped.)
...
4. **Label / eyebrow** — avoid the photo → avoid decor → if it overlaps the hero, move above → **else the placement solver walks relational + grid anchors** (P1 2b-i) and lands it as a quiet corner/edge eyebrow → and only a genuine solver refusal leaves it as an honest dead role (no silent drop). The same prior-then-solver path now governs the **date line** (drawn with the hero, priority just after it) and the **accent badge** (placed after support, before the logo).
...
### The placement solver (Part II) — `components/Generator.jsx` (spec `docs/element-placement-spec.md` §7)
| Knob | Current value | Where (anchor) | What changing it does |
|---|---|---|---|
| **Anchor priority order** | prior → relational → grid | the per-element candidate list at each solve site (date / eyebrow / badge) | Where the solver looks first. Candidate #1 is always the element's current (prior) spot, so a clean prior is byte-identical; relational anchors (below-hero, below-support, above-hero) come next; the platform-safe-inset 9-grid + corners last. |
| **Legibility ladder order** | ink → weight → robust face → size → band | `dateElementClass` / `eyebrowElementClass` `escalate.rungs` | The register-escalation sequence walked when a candidate is illegible. Date = serif (Romie 300 → 700 → Fira 600 → 700 size-up → band); eyebrow = caps (Syne 400 → 700 → Fira 600 → 700 size-up → band). The **badge** (`badgeElementClass`) is opaque → a single rung, no ladder. |
| **Class size floors** | date `MIN_FONT_PX.date`; eyebrow `MIN_FONT_PX.dateLabel`; badge from its `size` | `dateElementClass` / `eyebrowElementClass` `preferredPx`, `badgePillDim` | The smallest a solver-placed element sizes to (it only sizes *up* from the preferred; never below the class floor). |
| **Optical-weight ceiling** | `sqrt(maxV) > 0.14` forbids a thin register raw over photo texture | `escalate.opticalMax` (0.14), `heavyWeight` (600) | When a thin face on a busy photo is forced to escalate weight/face/band even at passing colour contrast (client ruling 2026-07-13). |
| **Snap grid** | the platform-safe-inset 9-grid + other elements' edges/centres + the element's original spot | `snapAnchors` / `snapCentre` (`SNAP_PX` 12 display-px) | What a manual drag magnetically snaps to. Already covers the solver's anchor set. The P1 owner rails landed (`2848696`): a drag of a solver-placed date/eyebrow/badge freezes its base (`{dx,dy,bx,by}`) so no re-solve can move the pin, and a pin in hard-restraint violation raises the `pinned-placement` fix:null ledger dot (Part V). |
| **Density budget** | *P2 — not built* | — | The per-format max-element-count / whitespace-floor governor (spec §4) is deferred to P2; today max freedom has no crowding-dot counterweight. |
...
- **The "12 archetypes" comment was stale (fixed, `5a548d5`).** Every catalog comment is now count-free ("trust the array"). The array holds 19 today.
...
- **The creative-plan route was orphaned (removed, `c2af8e1` + `d08e2ff`).** The dead route and its client handlers are deleted.
- **The headline-width floor was muted when obstacles pre-narrow the box (fixed, `5c1b01a`).** The starting-size target now fills ~0.92 of the RESOLVED hero box, so obstacle-narrowed headlines are bold within their zone. *Honest note:* across 13 measured photo-led cases the outcome was ±1px vs before — the per-archetype `heroCapFrac` start dominates in practice, so the visible-boldness symptom is plausible but unverified at the symptom level; the fix makes the floor *correct*, not necessarily *binding*.
- **PARTNER-fit-but-centred-outside-safe fell through (fixed, `235838a`).** The compose position now clamps into the safe rect while staying inside the shape's padded interior, else falls through to the OBSTACLE branch same-pass. Both-ways verified (before: eyebrow stranded mid-canvas; after: composed and clamped to the safe margin).
- **The format self-heal was dev-only (fixed, `257bc2d`).** Production now runs event-driven one-shot checks (after a format change settles and after media lands) with at most one corrective redraw, logging any firing to the feedback pipe (`kind:'format-fit-heal'`). The polling loop stays dev-only.
- **"Download all" silently skipped a failed format (fixed, `a5cba8c`).** Failures now surface as an honest retryable notice ("5 of 6 formats downloaded — Banner failed. Try again.") retrying only the failed formats, and log `kind:'export-error'`.
- **Still open (pre-existing, found by the batch-A sweep):** born-clean `safe-area-text` offenders on serif_word / manifesto / documentary × Story with short copy (3 of 228 guard cells, identical at baseline) — a follow-up work item, not a regression.
- **From slots to anchors — the date / eyebrow / badge adopt the placement solver (P1 — LANDED).** Three "Not shown in this layout" offenders now place on any design that has room instead of dropping when the archetype didn't author them: the **date** (`ae2a8c6`), the **eyebrow** (`e2ed8b8`), the **accent badge / pill** (`599f62c`). Each keeps its prior spot byte-identical when it lands and routes through the shared solver (Part II element vocabulary; Part IV solver knobs) only when the prior would drop. Verified live per element: DROP→PLACE across ≥2 archetypes × all 6 formats, prior-declared archetypes byte-identical, ladder fires end-to-end (probes `__woDateLadderProbe` / `__woEyebrowLadderProbe`), `__woArchStress` 114/0 + `__woBornCleanGuard` 342/0 unchanged from baseline, and autonomous no-content generations sprout nothing (born-clean holds). *Owner of the contract:* `docs/element-placement-spec.md`.
- **Owner rails for solver-placed elements (P1 owner rails — LANDED, `2848696`).** Both halves that Part V previously carried as PARTIAL now hold. (a) The **base-freeze pin guarantee** (law 5): the first explicit drag of a solver-placed date / eyebrow / badge freezes the solver's centre into the role's offset entry (`{dx,dy}` → `{dx,dy,bx,by}`, one atomic patch — undo / session snapshot / restore carry base + delta together), and a frozen role draws through a dedicated pinned path at exactly (base + delta) — no re-solve can move OR drop it (verified both ways per element: the same copy edit that measurably moves the undragged element leaves the pin at the same centre; undo returns it to the exact solver spot). Position is the pin; styling (ink, class-fit size) stays the system's free variable, and no pinned path ever reaches a band (§6a-safe by construction). This also closed the **badge jump**: the old skip-on-offset re-based a dragged badge on the raw authored prior, so a solver-moved badge jumped by (prior − solved) on the next render. (b) The **pinned-placement ledger dot**: the rescued eyebrow + drawn pill now join the audit's `ready.textBoxes` when unpinned; PINNED boxes are partitioned into `ready.pinned` instead (never double-reported, never auto-nudged by safe-area-text — M3), and one shared builder (`pinnedPlacementFinding`, `lib/audit-local.js`, the 4f9afcd fix:null pattern) raises the honest dot in both runLocalAudit and computeReadyVerdict only when a pin violates a hard restraint (platform action zone, measured collision). Its remedies are executable (law 2): "Put it back for me" (`roleOffset:{clear:true}` through the patch pipeline — the one sanctioned unpin), "Move it myself" (deep-link), "Keep it this way" (ack). Fresh generations carry `pinned:[]`, so born-clean holds (sweeps unchanged vs pre-slice baseline: `__woBornCleanGuard` 456/0, `__woArchStress` 114/0). *Honest caveats:* legacy delta-only entries from pre-freeze sessions keep the old translate-only behaviour until re-dragged once (a solver-moved badge with such an entry can still jump); pinned-vs-pinned overlaps are deliberately not counted (two owner placements crowding each other are both explicit intent); a pinned date draws only while a hero is present (the pin stays dormant otherwise, mirroring `logoFreePos`); the P3 remainder (live density-budget display, per-element size steps through the precedence invariant) is still pending.
- **Placed-element genes — DEFERRED to P2.** `buildGenes` still records the design-class gene set (archetype / variant / bg / treatment / register / postType / dimension / scene); adding "date/eyebrow/badge solver-placed" facts would extend the taste-loop gene contract (`GENE_KEYS`, the aggregator in `lib/preferences.js`, and the mirrored gene emission) — not cheap or self-contained, so it is deferred rather than half-wired.
