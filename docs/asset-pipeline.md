...
5. Record the pick into a rolling memory of the last twelve.
...
| **Recent-window length** | 12 | `RECENT_MAX` | How far back rhythm/anti-repeat looks. |
...
- **The taste memory is per-server-instance, not persisted.** `RECENT_PICKS` / `RECENT_VKINDS` / `LAST_BG` are module-level in memory — **they reset on every deploy and are shared across all users on the instance.** *Why it matters:* rhythm/anti-repeat "forgets" on deploy, and two users on one instance influence each other's variety.
- **The moodboard is captured but unread.** Moodboard pins carry no genes and the aggregator ignores them. *Why it matters:* a real user signal is being thrown away — the biggest quick win for the learning loop.
