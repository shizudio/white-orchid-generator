...
**Tier 0b — the required role paints anyway (client ruling 2026-07-26).** For a REQUIRED role (the heading/hero) Tier 0 gains a terminal rung: floor size → extra lines → roomier variant → **paint anyway**, complete and wrapped, at the readable floor, accepting an imperfect box. The role is never blanked and never cut. The overflow surfaces as `content.required-over-capacity` (warning) carrying the same action row the drop finding used to carry — *Tighten it for me* / a solver-verified roomier layout / *Edit it myself*. A layout with no slot for the required role must change (synthesize or reallocate), not swallow the content. Optional roles keep complete-or-absent unchanged. Born-clean is unaffected: Tier 1 writes generated copy to the measured budget, so a fresh design never reaches this rung.
