...
function ContentFieldsPanel({idPrefix,postType,content,heroRegister,eyebrow,pill,deadRoles=[],switchLayoutId,onField,onSwitchLayout,onEditRole,palette:B,fonts:F}){
  ...
  const input=(field,placeholder,maxLength,{mt=false,role,id,multiline=false,markers=false}={})=>{
    ...
    return <Tag id={id} className="wo-mfield" data-wo-role={role} aria-label={placeholder} placeholder={placeholder} maxLength={maxLength} value={shown} {...focusProps(role)} onChange={event=>handle(event.target.value)} style={{width:"100%",padding:"11px 14px",border:`1.5px solid ${B.ash}44`,borderRadius:10,fontSize:14,color:B.jet,boxSizing:"border-box",background:"#FAFAF7",fontFamily:F.body,marginTop:mt?8:0,...(multiline?{height:88,resize:"vertical"}:{})}}/>;
  };
  ...
  return <>
    ...
    {eyebrow?.visible&&<input className="wo-mfield" data-wo-role="eyebrow" aria-label="Eyebrow (small caps label)" placeholder="Eyebrow (small caps label)" maxLength={28} value={eyebrow.value??""} {...focusProps("eyebrow")} onChange={event=>onField("microLabel",event.target.value)} style={{width:"100%",padding:"11px 14px",border:`1.5px solid ${B.ash}44`,borderRadius:10,fontSize:14,color:B.jet,boxSizing:"border-box",background:"#FAFAF7",fontFamily:F.body,marginTop:8}}/>}
